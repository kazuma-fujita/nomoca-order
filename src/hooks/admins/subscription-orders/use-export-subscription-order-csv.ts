...
export const useExportSubscriptionOrderCSV = () => {
  const { now } = useNowDate();
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);

  const exportCSV = async (orders: ExtendedOrder<SubscriptionOrder>[]) => {
    setIsLoading(true);
    const exportData = [];
    try {
      if (!orders.length) {
      ...
      for (const order of orders) {
        for (const products of order.normalizedProducts) {
          const { total, taxes, subtotal } = calcTotalFromPriceAndQuantity(products.unitPrice, products.quantity);
          exportData.push({
            id: order.id,
            empty1: '',
            empty2: '',
