...
export const useExportSubscriptionOrderCSVAndCreateOrderHistory = () => {
  ...
  const exportSubscriptionOrderCSVAndCreateOrderHistory = async (orders: ExtendedOrder<SubscriptionOrder>[]) => {
    ...
    try {
      ...
      const { createdHistorySuccesses, createdHistoryFails } = await createOrderHistory(orders, now);
      ...
      const { updatedDeliveredAtSuccesses, updatedDeliveredAtFails } = await updateOrderDeliveredAt(orders, now);
      ...
      const notificationMailSubject =
        updatedDeliveredAtFails.length === 0
          ? '定期便のCSVを出力しました'
      ...
      const updatedDeliveredSuccessBody = `定期便CSV出力成功医院:\n${updatedDeliveredAtSuccesses
        ...
        .join('\n')}\n計:${updatedDeliveredAtSuccesses.length}件`;
      const updatedDeliveredFailedBody = `定期便CSV出力失敗件数:${updatedDeliveredAtFails.length}/${total}`;
      const sendMailSuccessBody = `メール送信成功件数:${sendMailSuccesses.length}/${updatedDeliveredAtSuccesses.length}`;
      const sendMailFailedBody = `メール送信失敗件数:${sendMailFails.length}/${updatedDeliveredAtSuccesses.length}`;
      const createdHistorySuccessBody = `定期便履歴データ登録成功件数:${createdHistorySuccesses.length}/${total}`;
      const createdHistoryFailedBody = `定期便履歴データ登録失敗件数:${createdHistoryFails.length}/${total}`;
      const notificationMailBody = `${updatedDeliveredSuccessBody}\n${updatedDeliveredFailedBody}\n\n\n${sendMailSuccessBody}\n${sendMailFailedBody}\n\n\n${createdHistorySuccessBody}\n${createdHistoryFailedBody}`;
