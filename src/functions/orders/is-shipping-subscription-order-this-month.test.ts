import { isShippingSubscriptionOrderThisMonthByLastDeliveredAtList } from './is-shipping-subscription-order-this-month';

// 配送日時配列がnull、もしくはundefinedならばfalseを返す
describe('isShippingSubscriptionOrderThisMonthByLastDeliveredAtList', () => {
  test('Returns false for an empty array list', () => {
    ...
    const isShippingOrder = isShippingSubscriptionOrderThisMonthByLastDeliveredAtList(lastDeliveredAtList, now);
  ...
  // 配送日時配列に一つでも当月と同じ配送年月があればtrueを返す
  test('Returns true if the current time and at least one ship date are in the same month', () => {
    const now = new Date('2023/01/01 00:00');
    // 配列の値はDBから取得するUTC時刻を想定。GMT+9のJSTを考慮する為、判定する-9の時刻をセット
